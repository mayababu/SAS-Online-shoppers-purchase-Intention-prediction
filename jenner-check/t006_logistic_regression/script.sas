...
ods html;
ods graphics on;
proc logistic data=online_shoppers2 plots(only)=(effect oddsratio);
...
model revenue(event="TRUE")= visitortype weekend quarter browser traffictype region operatingsystems Administrative
							 Administrative_Duration Informational Informational_Duration ProductRelated ExitRates
							 PageValues / details lackfit expb;
